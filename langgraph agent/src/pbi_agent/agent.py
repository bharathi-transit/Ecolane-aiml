...
def is_junk_query(query: str) -> bool:
    ...
    junk_patterns = [
        ...
        r"^how.*you",                               # How are you / How re you
        r"^what.*up",                               # Whats up / What is up
...
class AiDELLM:
    ...
    def invoke_text(self, prompt: str) -> str:
        ...
        try:
            ...
            raise ValueError(
                f"Empty/invalid JSON from AiDE completions (status {response.status_code})"
            )
...
def create_agent(resources: AgentResources):
    ...
    def build_query(state: AgentState) -> dict:
        direct_match = None
        try:
            explicit = parse_explicit_grouped_query(
                state.get("question", ""),
                resources.glossary_entries,
                resources.filter_entries,
            )
            if explicit:
                return explicit

            comparison_chart_template = detect_comparison_chart_pattern(
                state["question"], resources.visual_map
            )
            print(f"[DEBUG] Comparison chart template detected: {comparison_chart_template}")
            if comparison_chart_template:
                measure_names = comparison_chart_template.get("measures", [])
                matched_measures = []

                for measure_name in measure_names:
                    kpi_entry = match_kpi_from_question(
                        measure_name, resources.glossary_entries
                    )
                    if kpi_entry:
                        matched_measures.append(kpi_entry)

                if len(matched_measures) >= 2:
                    print(
                        f"[DEBUG] Returning comparison_chart query with {len(matched_measures)} measures"
                    )
                    return {
                        "kpi_name": "COMPARISON_CHART",
                        "filters": [],
                        "query_type": "comparison_chart",
                        "group_by": comparison_chart_template.get("group_by"),
                        "limit": 100,
                        "order": "ASC"
                        if comparison_chart_template.get("time_dimension")
                        else "DESC",
                        "table": None,
                        "columns": [],
                        "skip_defaults": False,
                        "comparison_measures": matched_measures,
                        "chart_type": comparison_chart_template.get("chart_type", "line"),
                    }

            comparison_kpis = detect_comparison_query(
                state["question"], resources.glossary_entries
            )
            if comparison_kpis and len(comparison_kpis) >= 2:
                ...
                if is_chart_query:
                    return {
                        ...
                        "columns": [],
                        "skip_defaults": False,
                        "comparison_measures": comparison_kpis,
                        "chart_type": "line",
                    }
                return {
                    "kpi_name": "COMPARISON",
            ...
            chart_template = detect_chart_template(
                state.get("question", ""), resources.visual_map
            )
            detail_template = detect_detail_template(
                state.get("question", ""), resources.visual_map
            )
            ...
            exact_kpi_match = _is_exact_kpi_request(
                state.get("question", ""), direct_match
            )

            if direct_match and exact_kpi_match and (
                not inferred_type or inferred_type == "kpi"
            ):
                return {
            ...
            if chart_template:
                template_type = (chart_template.get("query_type") or "").lower()
                query_type = inferred_type or ("pie" if template_type == "pie" else "barchart")
                return {
                    "kpi_name": chart_template.get("kpi_name"),
                    "filters": [],
                    "query_type": query_type,
                    "group_by": chart_template.get("group_by"),
                    "limit": chart_template.get("limit") or 10,
                    "order": "DESC",
                    "table": None,
                    "columns": [],
                    "skip_defaults": False,
            ...
            if detail_template:
                return {
                    "kpi_name": detail_template.get("__name__", "Detail"),
                    "filters": [],
                    "query_type": "detail",
                    "group_by": None,
                    "limit": detail_template.get("limit", 50),
                    "order": "DESC",
                    "table": detail_template.get("table"),
                    "columns": detail_template.get("columns", []),
                    "skip_defaults": False,
                }

            if direct_match and (not inferred_type or inferred_type == "kpi"):
                return {
                    "kpi_name": direct_match.get("name", ""),
                    "filters": [],
                    "query_type": "kpi",
            ...
            # Stop the process if the LLM returned empty/invalid JSON (Status 204)
            if not result:
                print("[DEBUG] LLM returned empty data. Triggering fallback.")
                return {
                    "fallback": True,
                    "response": "I'm sorry, I couldn't process the data from the server. Could you please rephrase your query?"
                }

            # Comprehensive fallback for non-KPI related inputs
            if result.kpi_name == "UNKNOWN" or not (direct_match or explicit or chart_template or detail_template):
                print("[DEBUG] Fallback triggered for non-KPI related input.")
                return {
                    "fallback": True,
                    "response": "I'm here to assist with KPI-related queries. Please specify a KPI or ask a relevant question."
                }

            if result.kpi_name == "UNKNOWN" or not any([direct_match, explicit, chart_template, detail_template, inferred_type]):
                return {
                    "fallback": True,
                    "response": "I'm here to help with KPI-related queries. Please provide details of the KPI or rephrase your question."
                }

            if result.kpi_name == "UNKNOWN" or not any([direct_match, explicit, chart_template, detail_template, inferred_type]):
                return {
                    "fallback": True,
                    "response": "Sorry, I didn't understand that. Could you please rephrase your question or specify the KPI you are interested in?"
                }

            if direct_match:
                kpi_name = direct_match.get("name", kpi_name)
            inferred_type = inferred_type or result.query_type

            return {
                "kpi_name": kpi_name,
                "filters": result.filters,
                "query_type": inferred_type or "kpi",
                "group_by": result.group_by,
                "limit": result.limit or 5,
                "order": result.order or "DESC",
                "table": result.table,
                "columns": result.columns,
                "skip_defaults": False,
            }
        except Exception as exc:
            # 1. Log the actual error for your debugging
            print(f"[ERROR] build_query failed: {exc}")
            
            # 2. Safety check: If we had a direct match before crashing, return it
            # Note: We use 'locals()' to safely check if the variable was ever created
            if 'direct_match' in locals() and direct_match:
                return {
                    "kpi_name": direct_match.get("name", ""),
                    "filters": [],
                    "query_type": "kpi",
            ...
            # 3. Final Fallback: Return a clean dictionary that the rest of the app expects
            # This prevents the "NameError" because we aren't using 'result_kpi'
            return {
                "fallback": True,
                "response": "I'm sorry, I encountered a technical error (Status 204 or Logic Error). Please try rephrasing your KPI request."
            }
