...
from .agent import (
    ...
    AgentResources,
    create_agent,
    handle_morning_ops_check,
    handle_otp_investigation,
)
...
def handle_morning_ops_check_cli(agent):
    """Handles the Morning Operations Check flow for the CLI."""
    result = handle_morning_ops_check(agent)
    print(f"\n--- {result['title']} ---")
    for kpi_result in result["kpis"]:
        print(f"- {kpi_result['kpi']}: {kpi_result['value']}")
...
def handle_otp_investigation_cli(agent):
    """Handles the OTP Investigation flow for the CLI."""
    # Call the centralized function to get the initial KPI data
    initial_results = handle_otp_investigation(agent)
    
    print(f"\n--- {initial_results['title']} ---")
    for kpi_result in initial_results["kpis"]:
        print(f"- {kpi_result['kpi']}: {kpi_result['value']}")

    # Proceed with the interactive CLI-specific part
    while True:
...
def main():
    ...
    while True:
        ...
        if user_input.lower() == "morning operations check":
            handle_morning_ops_check_cli(agent)
        elif user_input.lower() == "otp investigation":
            handle_otp_investigation_cli(agent)
